...
if (isset($_POST['submit'])) {
    ...
    $q="UPDATE mahasiswa SET nama='$nm',prodi='$pr',semester='$sm',email='$em',nim='$n' WHERE nim='".$_GET['nim']."'";
    mysqli_query($conn,$q);
    header("Location:../../index.php"); exit;
}
?>
<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"><title>Edit Mahasiswa</title><script src="https://cdn.tailwindcss.com"></script></head>
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
  <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
    ...
    <form method="POST">
      ...
      <input type="text" id="nim" name="nim" value="<?php echo htmlspecialchars($m['nim']) ?>" readonly class="w-full mb-4 px-3 py-2 border rounded bg-gray-100">

      <label class="block mb-1 text-gray-700">Nama</label>
      <input type="text" name="nama" value="<?php echo htmlspecialchars($m['nama']) ?>" required class="w-full mb-4 px-3 py-2 border rounded">
      ...
      <select name="prodi" id="prodi" required class="w-full mb-4 px-3 py-2 border rounded">
        ...
        <?php foreach(['Teknik Informatika','Sistem Informasi','Ilmu Komunikasi','Pariwisata'] as $prg): ?>
          <option value="<?php echo $prg ?>" <?php echo $m['prodi']==$prg?'selected':'' ?>><?php echo $prg ?></option>
        <?php endforeach; ?>
      </select>
      ...
      <input type="email" name="email" value="<?php echo htmlspecialchars($m['email']) ?>" required class="w-full mb-6 px-3 py-2 border rounded">
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    prodiSelect.addEventListener("change", function () {
      ...
      fetch(`get_nim.php?prodi=${encodeURIComponent(selectedProdi)}`)
        .then(response => response.text())
        .then(nimBaru => {
          if (nimBaru.trim()) {
            nimInput.value = nimBaru.trim();
          }
        })
        .catch(error => console.error("Gagal mengambil NIM:", error));
    });
  });
</script>
